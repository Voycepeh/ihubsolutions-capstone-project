...
| Full rotation generates unique orientations | A `30×20×10` item with `vertical_rotation=true` produces six unique axis assignments |
| Repeated dimensions remove duplicates | A `20×10×10` item produces three unique orientations rather than six duplicates |
| Upright rule keeps height on Z | With `vertical_rotation=false`, the original height never moves onto X or Y |
| Horizontal turn is still allowed | With `vertical_rotation=false`, `L×W×H` and `W×L×H` may both be tested |
| Sideways-only carton is rejected for upright item | A liquid-style item is not tipped sideways even when that would make it fit |
...
> A geometrically fitting orientation must still be rejected when it violates the item's `vertical_rotation` rule.
...
7. Orientation restrictions are business constraints and must never be relaxed by later optimization stages.
8. Historical iHub benchmark runs stay separate from normal unit tests because they measure solution quality and runtime rather than basic correctness.
