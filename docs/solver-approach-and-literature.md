...
An improved plan is kept only when it:

1. uses fewer cartons, or
2. uses the same number of cartons with lower total external carton volume.
...
The experiment is not simply whether Best Fit packs differently. It is whether the extra runtime produces a material business improvement in carton count or carton volume.
...
The supplied iHub service already operates at sub-second latency on the development sample, so packing quality must be evaluated together with runtime. The improvement stage should never make the system less reliable because the validated First Fit baseline is retained throughout.
