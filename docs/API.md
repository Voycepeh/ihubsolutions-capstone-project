...
The reusable package is `bin_packing_3d`. The public contract is designed so the same models can be used directly from Python and later exposed through FastAPI/OpenAPI without changing solver behavior.
...
A later FastAPI layer should stay thin:
...
See [`CONFIGURATION.md`](CONFIGURATION.md) for all settings and current defaults.
...
MVP 0 deliberately uses one physical item per carton. For each item it expands quantity, checks weight, generates the allowed orientations, applies carton clearance, selects the smallest-volume carton that physically fits the item, places it at `x=0, y=0, z=0`, and returns the chosen orientation and carton.
